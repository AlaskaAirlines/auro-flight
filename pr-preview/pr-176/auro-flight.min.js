...
`;class c extends i$2{constructor(){super(),this.stopover=false,this.canceled=false,this.destinationCanceled=false,this.partialCancel=false;}static get properties(){return {stopover:{type:Boolean,reflect:true},nextDay:{type:Boolean,reflect:true},iata:{type:String},duration:{type:String},canceled:{type:Boolean,reflect:true},destinationCanceled:{type:Boolean,reflect:true},partialCancel:{type:Boolean,reflect:true}}}static get styles(){return [i$5`${n}`,i$5`${r}`,i$5`${o}`]}static register(e="auro-flight-segment"){l.prototype.registerComponent(e,c);}firstUpdated(){l.prototype.handleComponentTagRename(this,"auro-flight-segment");}render(){const e$1={leg:true,layout:true,"leg--stopover":this.stopover,"leg--canceled":this.canceled||this.partialCancel};return u`
    ...
    `}}var d=i$5`.nonstop:before,.canceled:before{border-top-color:var(--ds-auro-flightline-segment-line-color)}:host([canceled]){--ds-auro-flightline-segment-line-color: var(--ds-basic-color-status-error, #e31f26)}
`,m=i$5`:focus:not(:focus-visible){outline:3px solid transparent}:host{display:block;isolation:isolate}.nonstop:before{content:"";height:var(--ds-size-25, .125rem);width:100%;position:relative;top:10px;border-width:var(--ds-size-25, .125rem) 0 0 0;border-style:solid}.canceled:before{border-style:dashed}.multiple ::slotted(*){display:block}@container (max-width: 320px){.multiple ::slotted(*){display:none}}.showNoStops{display:none}@container (max-width: 320px){.showNoStops{display:block}}.slotContainer{display:flex;justify-content:space-around;min-height:var(--ds-size-300, 1.5rem);container-type:inline-size}
`;class g extends i$2{constructor(){super(),this.canceled=false,this.hasCanceledSegment=false,this.firstSegmentCanceled=false,this.lastSegmentCanceled=false,this.runtimeUtils=new l;}static get properties(){return {canceled:{type:Boolean,reflect:true},hasCanceledSegment:{type:Boolean,reflect:true},firstSegmentCanceled:{type:Boolean,reflect:true},lastSegmentCanceled:{type:Boolean,reflect:true}}}static get styles(){return [m,d,o]}static register(e="auro-flightline"){l.prototype.registerComponent(e,g);}firstUpdated(){this.runtimeUtils.handleComponentTagRename(this,"auro-flightline");}containsCanceledSegment(){const e=this.querySelectorAll("auro-flight-segment, [auro-flight-segment]");for(let s=0;s<e.length;s+=1){const t=e[s];this.canceled&&(t.canceled=true,t.destinationCanceled=true),(t.canceled||t.hasAttribute("canceled"))&&(this.hasCanceledSegment=true,0===s&&(this.firstSegmentCanceled=true)),t.hasAttribute("destinationCanceled")&&s===e.length-1&&(this.lastSegmentCanceled=true);}}render(){const e$1=this.children.length>1,s={slotContainer:true,nonstop:!this.children.length,multiple:e$1,canceled:this.canceled};return x`
      <div class="${e(s)}">
...
var colorFlightHeaderCss = i$5`:host{color:var(--ds-auro-flight-header-text-color)}.daysChanged{color:var(--ds-auro-flight-header-days-changed-text-color)}
`;
...
var colorFlightMainCss = i$5`.departureTime{color:var(--ds-auro-flight-time-text-color)}.departureStation{color:var(--ds-auro-flight-station-text-color)}.arrivalTime{color:var(--ds-auro-flight-time-text-color)}.arrivalStation{color:var(--ds-auro-flight-station-text-color)}.util_lineThrough{color:var(--ds-auro-flight-cancelled-station-text-color)}
`;
